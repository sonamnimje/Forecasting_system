...
def load_and_clean(path: str = "data/Sales_data.csv") -> pd.DataFrame:
    ...
    - state
    - date
    - total (will be renamed to sales)
    - category (optional → ignored)
    """
    ...
    # Fill missing weekly dates per state
    ...
    # Interpolate missing sales values per state
    df["sales"] = (
    ...
    # Remove states with completely missing sales
    invalid_states = []
    ...
    if invalid_states:
        print(
            f"Removing states with completely missing sales data: "
            f"{invalid_states}"
        )
...
def _fill_missing_dates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Fix:
    Previously global min/max created unnecessary NaN rows.
    Now each state gets only its own actual range.
    """
    frames = []
